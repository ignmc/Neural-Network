...
class NeuralLayer:
    ...
    def update_weights(self, inputs, lr):
        ...
        if self.is_first_layer():
            ...
            previous_inputs = [previous_neuron.output for previous_neuron in self.previous_layer]

        for neuron in self.neurons:
            neuron.adjust_weights_with_input(previous_inputs, lr)
            neuron.adjust_bias_with_learning_rate(lr)

        if self.next_layer is not None:
            self.next_layer.update_weights(inputs, lr)
